    ...
    try:
        ...
        if key:
            ...
            url = (f"https://api.twelvedata.com/time_series"
                   ...
                   f"&outputsize={cantidad}&apikey={key}")
            resp = requests.get(url, timeout=15).json()
            if "values" not in resp:
                print(f"Twelve Data error [{interval}]: {resp.get('message', resp)}")
                return None
            ...
            time.sleep(8)  # respetar límite de 8 llamadas/min del plan gratuito
            return df
